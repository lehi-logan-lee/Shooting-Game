package com.example.student.shootinggame;
...
public class ShootingView extends SurfaceView
    ...
    private MediaPlayer m_mediaPlayerMeteo = null;
    //シーン定数(1)
    ...
    public ShootingView(Activity activity) {
        super(activity);
        m_mediaPlayerMeteo = MediaPlayer.create(activity, R.raw.bomb);
        //ビットマップの読み込み
        for (int i = 0; i < 9; i++) {
                    ...
                    //衝突
                        ...
                        for (int j = meteos.size() - 1; j >= 0; j--) {
                            ...
                            if (Math.abs(pos0.x - pos1.x) < 50 &&
                                ...
                                meteos.remove(j);
                                // 再生中かどうか？
                                if (m_mediaPlayerMeteo.isPlaying()) {
// （再生中の場合は）再生停止
                                    m_mediaPlayerMeteo.pause();
                                }
// 再生位置を先頭に戻す
                                m_mediaPlayerMeteo.seekTo(0);
// 再生開始
                                m_mediaPlayerMeteo.start();
                                score += 30;
                                break;
                            }
                        }
